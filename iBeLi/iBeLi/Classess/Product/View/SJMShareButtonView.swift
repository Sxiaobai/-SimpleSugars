...
class SJMShareButtonView: UIView {
    // 图片数组
    ...
    private func setupUI() {
        ...
        let xMargin: CGFloat = (SCREEN_W - 20 - 2 * buttonStartX - CGFloat(maxCols) * buttonW) / CGFloat(maxCols - 1)
    ...
    func shareButtonClick(button: UIButton) {
        if let shareButtonType = SJMShareButtonType(rawValue: button.tag) {
