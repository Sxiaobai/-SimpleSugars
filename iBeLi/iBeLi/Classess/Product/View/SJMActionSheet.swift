...
class SJMActionSheet: UIView {
    
    class func show() {
        let actionSheet = SJMActionSheet()
        actionSheet.frame = UIScreen.mainScreen().bounds
        actionSheet.backgroundColor = SJMColor(0, g: 0, b: 0, a: 0.6)
    ...
    override func layoutSubviews() {
        ...
        UIView.animateWithDuration(kAnimationDuration) {
            self.bgView.y = SCREEN_H - self.bgView.height
        }
    }
    ...
    private lazy var bgView: UIView = {
        let bgView = UIView()
        bgView.frame = CGRectMake(0, SCREEN_H, SCREEN_W, 280)
        return bgView
    }()
    // 上部 view
    ...
    private lazy var shareLabel: UILabel = {
        ...
        shareLabel.text = "分享到"
        shareLabel.textColor = SJMColor(0, g: 0, b: 0, a: 0.7)
        shareLabel.textAlignment = .Center
        return shareLabel
    }()
    // 6个按钮
    private lazy var shareButtonView: SJMShareButtonView = {
        let shareButtonView = SJMShareButtonView()
        shareButtonView.frame = CGRectMake(0, 30, SCREEN_W - 20, kTopViewH - 30)
    ...
    private lazy var cancelButton: UIButton = {
        ...
        cancelButton.setTitleColor(SJMColor(37, g: 142, b: 240, a: 1.0), forState: .Normal)
